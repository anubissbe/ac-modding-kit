...
| Building availability and catalogue probes | Runtime-observed, build-specific | Culture plus Knowledge/XP is evidenced; direct `Year`/`HistoryRangeYear` gating is unproven. Paired requirement omission and a positive ones-vector are backed-up live-loose diagnostics only; zero `ConstitutionCount` disables the tested catalogue entry |
...
| Workshop packet and post-publish ID sync | Supported safety tooling | Hashes one exact action/candidate, optionally verifies generated ZIP members, preserves immutable predecessor IDs, and remains dry-run-first |
| Automatic Steam Workshop publication | Intentionally unsupported | Use the current in-game UI; immediately before each Publish/Update require single-use action/candidate/target/time confirmation with exact item IDs, visibility, file list, and SHA-256 hashes |
